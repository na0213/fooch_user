...
<div class="mb-12">
  <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
    ...
    <form action="{{ route('conversation.send', ['order' => $order->id]) }}" method="POST" class="mt-8">
        @csrf
        <textarea name="message" class="w-full p-2 border rounded" placeholder="メッセージを入力..."></textarea>
        <button type="submit" class="bg-blue-500 text-white px-4 py-2 rounded mt-2">送信</button>
    </form>
  </div>
</div>
