...
class Payment extends Model
{
     ...
     */
    public static function getDefaultcard($user)
    {
        \Stripe\Stripe::setApiKey(\Config::get('payment.stripe_secret'));
        ...
        if (!is_null($user->stripe_id)) {
            ...
            Log::info('Customer data:', ['customer' => $customer]); // この行を追加
    
            if (isset($customer['default_source']) && $customer['default_source'] && isset($customer->sources) && isset($customer->sources->data)) {
                $card = $customer->sources->data[0];
                Log::info('Card data:', ['card' => $card]); // この行を追加
